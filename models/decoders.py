...
from layers.hyp_layers import HypMlrBm
...
class LinearDecoder(Decoder):    
    ...
    def decode(self, x, adj):
        h = self.manifold.proj_tan0(self.manifold.logmap0(x, c=self.c), c=self.c) # proj_tan0 is the identity function. 
...
class HypMLRDecoder(Decoder):
    """
    Hyperbolic MLP Decoder for Hyperbolic node classification models.
    """
    def __init__(self, c, args):
        super(HypMLRDecoder, self).__init__(c)
        self.manifold = getattr(manifolds, args.manifold)()
        self.input_dim = args.dim
        self.output_dim = args.n_classes
        self.cls = HypMlrBm(self.manifold, self.input_dim, self.output_dim, self.c)        
        self.decode_adj = False
        
    def decode(self, x, adj):
        return super(HypMLRDecoder, self).decode(x, adj)    
...
model2decoder = {
    ...
    'GAT': GATDecoder,
    'HNN': LinearDecoder,
    'HGCN': HypMLRDecoder,    # LinearDecoder
    'MLP': LinearDecoder,
    'Shallow': LinearDecoder, # HypMLRDecoder
}
